...
## Профилирование через SPX
Для вклчюения профилировщика необходимо зайти на http://<домен-сайта>/?SPX_KEY=dev&SPX_UI_URI=/
...
## Дебаг почты через Mailhog

Настраиваем SMPT битрикса по документации - https://dev.1c-bitrix.ru/learning/course/index.php?COURSE_ID=43&LESSON_ID=23612
![img.png](./docs/img/img_7.png)
`Email` - та почта с которой шлются письма, обычно указана в почтовом шаблоне
`Имя отправителя` - любое
`Для всех пользователей` - включить
`Сервер` - `mailhog`
`Порт` - `1025`
`Логин` - любой
`Пароль` - пустой
